#!/usr/bin/env python3
"""
Optuna-based Hyperparameter Search for DFU Transition Prediction Model
(Direct Transition Label Training)

DIFFERENCE FROM optuna_search.py:
- This script trains DIRECTLY on transition labels (Unfavorable/Acceptable/Favorable)
- The original optuna_search.py trains on phase labels (I/P/R) then converts to transitions
- Direct training eliminates conversion error and is more accurate
...
These thresholds use CUMULATIVE phase duration (how long wound has been in that phase
across consecutive same-phase appointments), not just days between appointments.

ONSET VALUE CORRECTION:
- Onset values are replaced with median values grouped by phase at first appointment (Appt 0)
- For each wound, we find the phase at its first clinic visit
- Group all wounds by their phase at Appt 0 (I, P, or R)
- Calculate median onset for each phase group
- Replace all onset values based on the phase at first appointment
- This handles potentially unreliable onset values by using phase-based estimates
- Assumes I (Inflammatory) at wound onset for transition labeling

FIRST APPOINTMENT CHRONICITY (searchable parameter):
- skip_first_appt_chronicity: If True, skip chronicity check for first appointments
- When True: I→I at first appointment is always Acceptable (regardless of onset)
- When False: I→I at first appointment uses onset_days for chronicity check
- Effect: Reduces class imbalance from ~2.24 ratio to ~1.26 ratio
- Rationale: Onset values are often unreliable for first appointments

Output Format:
- Produces the same output files as optuna_search.py:
...
- Multi-objective support: optimize for multiple metrics simultaneously
- Resume capability via Optuna's built-in storage
- Periodic checkpoint saving (JSON/CSV) every N trials

Usage:
    python optuna_search_transition_labels.py [--n-trials N] [--n-jobs N] [--resume]
...
Arguments:
    ...
    --optimize-for: Comma-separated list of metrics to optimize

RECOMMENDATION:
    python optuna_search_transition_labels.py --n-trials 500 --optimize-for combined_score
"""
...
# Features to EXCLUDE when training on transitions
# ONLY exclude features that would cause data leakage (i.e., reveal the target)
PHASE_FEATURES_TO_EXCLUDE = [
    'Next_Healing_Phase',  # This IS the target - we derive transition labels from it
]
# NOTE: We KEEP these features because they are historical/contextual, not leaky:
#   - Previous_Phase: Essential context (transition from P is different from I)
#   - Initial_Phase: Historical info
#   - History_Phase_*: Historical phase counts
#   - Phase_Improvements/Regressions_Count: Historical transition counts
#   - Cumulative_Phase_Duration: Duration in current phase (used for chronicity)


# ============================================================================
# Onset Value Correction Functions
# ============================================================================
# Many onset values may be unreliable. We replace them with median values
# grouped by the phase observed at the first appointment (Appt 0).
# This provides more consistent onset estimates based on clinical presentation.
...
def compute_median_onset_by_first_phase(df: pd.DataFrame, preprocessor) -> Dict[int, float]:
    ...
    Logic:
    - For each unique wound (Patient#, DFU#), find Appt 0 and its phase
    - Group all wounds by their phase at Appt 0
    - Calculate median onset for each phase group

    Parameters
    ----------
    df : pd.DataFrame
        Processed dataframe with samples (may contain augmented data)
    preprocessor : DFUNextAppointmentPreprocessor
        Preprocessor with access to raw data

    Returns
    -------
    Dict[int, float]
        Mapping from phase (0=I, 1=P, 2=R) to median onset days
    """
    raw_df = preprocessor.df

    # Find all first appointments (Appt 0) and their phases
    first_appts = []
    for (patient, dfu), group in raw_df.groupby(['Patient#', 'DFU#']):
        ...
        # Get phase at first appointment
        phase = first_row.get('Healing_Phase', np.nan)
        if pd.isna(phase):
            phase = first_row.get('Initial_Phase', np.nan)

        if not pd.isna(phase) and not pd.isna(onset):
            first_appts.append({
                'Patient#': patient,
                'DFU#': dfu,
                'phase_at_appt0': int(phase),
                'onset_days': float(onset)
            })

    if not first_appts:
        print("  Warning: No first appointments found for onset median computation")
        return {0: 90.0, 1: 90.0, 2: 90.0}  # Default fallback

    first_df = pd.DataFrame(first_appts)

    # Compute median onset for each phase
    median_onset_by_phase = {}
    for phase in [0, 1, 2]:  # I, P, R
        ...
        if len(phase_onsets) > 0:
            ...
            # Fallback to overall median if no samples for this phase
            median_onset_by_phase[phase] = float(first_df['onset_days'].median())

    phase_names = {0: 'I', 1: 'P', 2: 'R'}
    print(f"  Median onset by phase at Appt 0:")
    for phase, median in median_onset_by_phase.items():
        count = len(first_df[first_df['phase_at_appt0'] == phase])
        print(f"    Phase {phase_names[phase]}: {median:.1f} days (n={count})")
...
def get_phase_at_first_appt_map(preprocessor) -> Dict[Tuple[Any, Any], int]:
    ...
    Dict[Tuple[Any, Any], int]
        Mapping from (Patient#, DFU#) to phase at Appt 0
    """
    raw_df = preprocessor.df
    phase_map = {}
...
def replace_onset_with_median(df: pd.DataFrame, preprocessor,
    ...
    """
    Replace onset values in the dataframe with median values based on phase at first appointment.

    For each sample, looks up the phase at the first appointment for that wound,
    then uses the corresponding median onset value.

    Parameters
    ----------
    df : pd.DataFrame
        Dataframe with samples (may contain augmented data)
    preprocessor : DFUNextAppointmentPreprocessor
        Preprocessor (not used directly, kept for API consistency)
    median_onset_by_phase : Dict[int, float]
        Mapping from phase to median onset days
    phase_at_first_appt_map : Dict[Tuple[Any, Any], int]
        Mapping from (Patient#, DFU#) to phase at Appt 0
    silent : bool
    ...
    -------
    pd.DataFrame
        Dataframe with corrected onset values
    """
    df = df.copy()
    ...
    if onset_col not in df.columns:
        if not silent:
            print(f"  Warning: No onset column found, cannot replace onset values")
        return df

    original_onset = df[onset_col].copy()
    new_onset = original_onset.copy()

    # Replace onset values based on phase at first appointment
    for idx in df.index:
        ...
        if phase_at_first is not None and phase_at_first in median_onset_by_phase:
            new_onset.loc[idx] = median_onset_by_phase[phase_at_first]

    df[onset_col] = new_onset

    # Report changes
    if not silent:
        n_changed = (original_onset != new_onset).sum()
        print(f"  Replaced {n_changed}/{len(df)} onset values with phase-based medians")
...
# NOTE: This function is NOT USED in direct transition training.
# When training directly on transition labels, no conversion is needed.
# Kept here for reference/backwards compatibility only.
def _convert_phase_proba_to_transition_proba_UNUSED(
...
def calculate_all_metrics_direct(
    ...
    This is a simplified version for when we train directly on transition labels.
    No conversion from phase labels needed.
    ...
    y_pred_trans : np.ndarray
        Predicted transition labels
    y_pred_proba : np.ndarray
        Predicted transition probabilities (n_samples, 3)
...
def calculate_combined_score_calibration_minority(metrics: Dict[str, float]) -> float:
    ...
    Optimizes for:
    - ECE (Expected Calibration Error) - reliable probability estimates
    - MCE (Maximum Calibration Error) - no severely miscalibrated bins
    - f1_acceptable - often the minority class after skip_first_appt_chronicity
    - f1_unfavorable - critical for clinical decisions

    This is useful when you need both well-calibrated probabilities AND
    good performance on the harder-to-predict transition classes.
    """
    weights = {
        'ece_inv': 0.25,             # Expected calibration error (lower is better)
        'mce_inv': 0.25,             # Maximum calibration error (lower is better)
        'f1_acceptable': 0.25,       # Often minority class, harder to predict
        'f1_unfavorable': 0.25,      # Critical for clinical decisions
    }
    return _compute_weighted_score(metrics, weights)
...
# ============================================================================
# Training Function for DIRECT TRANSITION LABEL PREDICTION
# ============================================================================
...
def compute_transition_labels_for_df(df: pd.DataFrame,
    ...
    ----------
    df : pd.DataFrame
        Dataframe with columns: Next_Healing_Phase, Previous_Phase, Days_To_Next_Appt,
        Onset (Days), Cumulative_Phase_Duration
    ...
    # Compute transition labels using fixed thresholds
...
def train_and_evaluate_config(
    ...
    DIRECT TRANSITION TRAINING: This trains directly on transition labels
    (Unfavorable/Acceptable/Favorable) instead of phase labels (I/P/R).

    Uses corrected onset values (median by phase at first appointment) when provided.
    Note: Onset medians are computed once upfront and passed in.
    """
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(unique_patients)):
        ...
        # Apply onset correction to validation data if onset maps are provided
        if median_onset_by_phase and phase_at_first_appt_map:
            val_df = replace_onset_with_median(
                val_df, preprocessor, median_onset_by_phase, phase_at_first_appt_map,
                silent=True  # Don't print during training loops
            )

        # Compute TRANSITION LABELS (not phase labels) using fixed thresholds
        ...
        # Get features (excluding phase-related features)
        X_train = train_df[feature_cols].copy()
        X_val = val_df[feature_cols].copy()

        # Compute sample weights based on transition labels
        # Use class weights inversely proportional to class frequency
        unique_trans, counts = np.unique(y_train_trans, return_counts=True)
        class_weights_dict = {cls: len(y_train_trans) / (len(unique_trans) * count)
                              for cls, count in zip(unique_trans, counts)}
        ...
        # Apply resampling on transition labels directly
        if config['resampling'] != 'none':
            try:
                ...
                unique_trans, counts = np.unique(y_train_resampled, return_counts=True)
                class_weights_dict = {cls: len(y_train_resampled) / (len(unique_trans) * count)
                                      for cls, count in zip(unique_trans, counts)}
        ...
        for model_idx in range(n_models):
            ...
            try:
                if use_focal_loss and use_calibration and len(np.unique(y_train_resampled)) >= 2:
                    ...
                    model = ExtraTreesClassifier(
                        ...
                        max_samples=config.get('max_samples') if config['bootstrap'] else None,
                        random_state=model_seed, n_jobs=n_jobs
                    )
            ...
            # Ensure 3-class probability output
        ...
        # This learns separate isotonic calibration curves for each class
        # using the training set predictions, then applies to validation
        use_per_class_calibration = config.get('use_per_class_calibration', False)
        if use_per_class_calibration:
            try:
                # Get predictions on training data (pre-resampled) to learn calibration maps
                # Use the last trained model for calibration (or could use ensemble)
                train_proba_for_cal = model.predict_proba(X_train_scaled)

                # Ensure 3-class probability output for calibration set
    ...
    # Calculate metrics directly on transition predictions (no conversion needed!)
...
class OptunaObjective:
    ...
    DIRECT TRANSITION TRAINING VERSION:
    - Trains on transition labels (Unfavorable/Acceptable/Favorable) directly
    - Uses FIXED thresholds: I_THRESHOLD=21, P_THRESHOLD=42 (not searched)
    - Excludes phase-related features from training
    - Uses corrected onset values (median by phase at first appointment)
    """
    ...
    def __init__(self, preprocessor, augmentation_datasets, n_folds, n_jobs,
        ...
        self.median_onset_by_phase = median_onset_by_phase or {}
        self.phase_at_first_appt_map = phase_at_first_appt_map or {}

        # Filter out phase-related features from augmentation datasets
        self.augmentation_datasets = {}
        for aug_type, aug_data in augmentation_datasets.items():
            filtered_features = [f for f in aug_data['feature_cols']
                                 if f not in PHASE_FEATURES_TO_EXCLUDE]
            self.augmentation_datasets[aug_type] = {
                'df_processed': aug_data['df_processed'],
                'patient_cluster_map': aug_data['patient_cluster_map'],
                'feature_cols': filtered_features
            }
            print(f"  {aug_type}: Filtered {len(aug_data['feature_cols'])} -> {len(filtered_features)} features")

    def __call__(self, trial: optuna.Trial) -> float:
        self.trial_count += 1

        # Sample hyperparameters (NO threshold parameters - they're fixed!)
        config = {
            ...
            # Augmentation (only 'none' and 'safe_sequential' supported)
            ...
            # If True, skip chronicity check for first appointments (I→I at Appt 0 = Acceptable)
            # This reduces class imbalance from ~2.24 to ~1.26
        ...
        try:
            ...
            # Store fixed thresholds (not searched, but important for reproducibility)
            trial.set_user_attr('config_i_threshold', I_THRESHOLD)
            trial.set_user_attr('config_p_threshold', P_THRESHOLD)
            trial.set_user_attr('config_r_threshold', R_THRESHOLD)
...
def run_optuna_search(
    ...
    print("="*80)
    print("OPTUNA HYPERPARAMETER SEARCH FOR DFU TRANSITION PREDICTION")
    print("(Direct Transition Label Training)")
    print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*80)
    print(f"\nFIXED THRESHOLDS (not searched):")
    print(f"  I_THRESHOLD = {I_THRESHOLD} days")
    print(f"  P_THRESHOLD = {P_THRESHOLD} days ({P_THRESHOLD/7:.0f} weeks)")
    ...
    # Step 1: Compute median onset by phase at first appointment (once for all datasets)
    print("\n  Computing median onset values by phase at first appointment...")
    # Use the raw data to compute medians - this is done before augmentation
    # to ensure we only look at real first appointments
    median_onset_by_phase = compute_median_onset_by_first_phase(None, preprocessor)
    phase_at_first_appt_map = get_phase_at_first_appt_map(preprocessor)
    print(f"    Mapped {len(phase_at_first_appt_map)} unique wounds to their phase at Appt 0")

    augmentation_datasets = {}

    for aug_type in ['none', 'safe_sequential']:
        print(f"\n  Creating dataset for augmentation: {aug_type}")
        ...
        # Step 2: Replace onset values with phase-based medians
        print(f"    Replacing onset values with phase-based medians...")
        df_processed = replace_onset_with_median(
            df_processed, preprocessor,
            median_onset_by_phase, phase_at_first_appt_map
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Optuna Hyperparameter Search for DFU Transition Model (Direct Training)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
DIRECT TRANSITION TRAINING:
  This script trains directly on transition labels (Unfavorable/Acceptable/Favorable)
  instead of phase labels (I/P/R). Fixed thresholds: I=21 days, P=42 days (6 weeks).

RECOMMENDATION:
  For most use cases, optimize for just 'combined_score':
      python optuna_search_transition_labels.py --n-trials 500 --optimize-for combined_score

Examples:
  # Single metric (recommended):
  python optuna_search_transition_labels.py --n-trials 500 --optimize-for combined_score

  # Multiple metrics:
  python optuna_search_transition_labels.py --n-trials 500

  # Custom metrics:
  python optuna_search_transition_labels.py --n-trials 500 --optimize-for combined_score,f1_macro
        """
    )
